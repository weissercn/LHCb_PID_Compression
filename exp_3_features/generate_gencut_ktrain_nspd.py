"""
This file  generate_gencut_ktrain.py is like generate_gencut.py but running on a model trained on half the K data and generates new MC from a trained GAN. However, it allows us to regenerate an event if it doesn't pass a cut.    
Also, evt_nspd is added to the training
Run like:
...
# 1. GAN
MODEL_NAME = "GAN_Kramer_pid{}_ktrain_nspd".format(inv_pids[args.particle_type])
