...
# 1. GAN
MODEL_NAME = "UncondFlow_Kramer_pid{}_ksttrain_nspd".format(inv_pids[args.particle_type])
...
while np.sum(np.logical_not(passcut_mask))>0:
    ...
    output_gen_part = flowbest.sample(input_data.shape[0]).numpy()
    #output_gen_part = flowbest.sample(1, context=torch.tensor(input_data,dtype=torch.float32)).numpy().reshape((input_data.shape[0],-1))
