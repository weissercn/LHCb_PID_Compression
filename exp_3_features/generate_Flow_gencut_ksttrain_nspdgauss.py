"""
This file generate_FLOW_gencut_ksttrain_nspdgauss.py is like generate_FLOW_gencut_ksttrain_nspd.py but uses a cdf -based gaussian scaler [sklearn.preprocessing.QuantileTransformer(output_distribution='normal')] before using flows.
Also, this scriot runs on the gpu.
Run like:
...
# 1. GAN
MODEL_NAME = "Flow_Kramer_pid{}_ksttrain_nspdgauss".format(inv_pids[args.particle_type])
...
data_scaled = pd.DataFrame(gauss_scaler.transform(data_scaled), columns=data_scaled.columns)
...
data_scaled_test = pd.DataFrame(gauss_scaler.transform(data_scaled_test), columns=data_scaled_test.columns)
#print("data_scaled_test ", data_scaled_test.shape, "\n",data_scaled_test)
...
while np.sum(np.logical_not(passcut_mask))>0:
    ...
    context_gen = torch.tensor(input_data.to_numpy(), dtype=torch.float32)
    output_gen_part = flowbest.sample(1, context=context_gen).numpy().reshape((input_data.shape[0],-1))
    
    #context_gen = torch.tensor(input_data.to_numpy(), dtype=torch.float32).to(device)
    #output_gen_part = flowbest.sample(1, context=context_gen).cpu().numpy().reshape((input_data.shape[0],-1))
...
output_transformed = gauss_scaler.inverse_transform(np.concatenate([data_scaled[vars_list_aux].values, output_gen], axis=1))
