"""
This file  generate_gencut_ksttrain_nspdsigpnnrestrnomix.py is like generate_gencut_ksttrain_nspdsigpnnrestr.py but training on K from Kstee and K from Kstmm separately.  
Also, evt_nspd is added to the training and an inverse sigmoid function is applied to GS0x7  (trks_ProbNNk) to not have all values close to 0 or 1. 
Run like:
...
parser.add_argument('particle_type', choices=['kaone', 'kaonm', 'electron', 'muon'],
...
# build-in parameters
pids = {'ke':'kaone', 'km':'kaonm', 'e': 'electron', 'm': 'muon'}
inv_pids = {v:k for k,v in pids.items()}
pid = inv_pids[args.particle_type]
in_columns = ["S6aux0", 'S0aux7', 'S0aux6', 'S3aux0', 'S2aux0', 'S5aux0']
out_columns = ['0', '1', '2']

vars_list_input_orig = ['GS3x1', 'GS3x0', 'GS0x7']
vars_list_input = ['GS3x1', 'GS3x0', 'GS0x7_invsigmoid'] #transforming input features before generating is beneficial
...
# 1. GAN
MODEL_NAME = "GAN_Kramer_pid{}_ksttrain_nspdsigpnnrestrnomix".format(inv_pids[args.particle_type])
...
data['GS0x7_invsigmoid'] = data['GS0x7'].apply(inv_sigmoid)
data.drop("GS0x7", axis=1, inplace=True)
...
#data_scaled_test['GS0x7'][0] = inv_sigmoid(0) #division by zero
#data_scaled_test['GS0x7'][1] = inv_sigmoid(1) #division by zero
...
GS0x7_0 = -np.inf #data_scaled_test['GS0x7'][0]
GS0x7_1 = np.inf #data_scaled_test['GS0x7'][1]
...
df =pd.DataFrame(np.concatenate([gan_output, data[vars_list_aux]], axis=1), columns=vars_list_input+vars_list_aux)
df.insert(OUT_DIM-1+1,'GS0x7',df['GS0x7_invsigmoid'].apply(sigmoid))
df.drop("GS0x7_invsigmoid", axis=1, inplace=True)
